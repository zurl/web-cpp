 ...
 * @return {{ code: string, map: SourceMapGenerator }}
 */
function replaceWithSourceMap(sourceFileName, source, replacements, generatedFileName) {
...
`;
    ...
    const nodes = parser.parse(source).map(node => new SourceNode(node.line, node.column, sourceFileName, node.source));
 ...
 * @return {string}
 */
function balanceCurlyBraces(string) {
    if (string.includes('{')) {
        return string + '/*}*/'
