...
import {ArrayType, LeftReferenceType} from "../type/compound_type";
...
export class WAddressHolder extends WExpression {
    ...
    public static fromVariable(ctx: CompileContext, v: Variable, node: Node) {
        if (v.type instanceof ArrayType) {
            return new WAddressHolder(v.location, v.addressType, node.location)
                .createLoadAddress(ctx);
        } else if (v.type instanceof LeftReferenceType) {
            return new WAddressHolder(
                    new WAddressHolder(v.location, v.addressType, node.location)
                    .createLoad(ctx, PrimitiveTypes.int32),
