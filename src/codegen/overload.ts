...
export function doStrictTypeMatch(ctx: CompileContext, dst: Type, src: Type): boolean {
...
export function doWeakTypeMatch(ctx: CompileContext, dst: Type, src: Type): boolean {
    if (dst instanceof ReferenceType) {
        dst = dst.elementType;
    }
    if (src instanceof ReferenceType) {
    ...
    return isCompatWith(ctx, dst, src);
}

export function doFunctionFilter(ctx: CompileContext, func: FunctionEntity, argus: Type[],
                                 funcs: FunctionLookUpResult,
                                 matcher: (ctx: CompileContext, dst: Type, src: Type) => boolean): boolean {
    if (func.type.isMemberFunction()) {
        ...
        return isCompatWith(ctx, first.elementType, funcs.instanceType) &&
            func.type.parameterTypes.slice(1).every((t, i) => matcher(ctx, t, argus[i]));
    } else {
        return func.type.parameterTypes.every((t, i) => matcher(ctx, t, argus[i]));
    }
}
...
export function doFunctionOverloadResolution(ctx: CompileContext,
    ...
    const f2 = f1.filter((func) => doFunctionFilter(ctx, func, argus, funcs, doStrictTypeMatch));
    ...
    const f3 = g0.filter((func) => doFunctionFilter(ctx, func, argus, funcs, doWeakTypeMatch));
    ...
    for (const item of templates) {
        ...
        const params = deduceFunctionTemplateParameters(ctx, item, mockFunctionType, funcs.templateArguments, true);
        if (params !== null) {
